...
// 1. Auto-détecter la version de base
...
// 2. Copier récursivement le dossier template
...
log(`   1. cd ${themeName}`);
log(`   2. Modifier le fichier package.json (homepage, maintainers, etc.)`);
log(`   3. Personnaliser src/scss/variables.scss`);
log(`   4. Commencer à développer votre thème\n`);

log("🚀 Commandes utiles:", "bright");
log(`   pnpm --filter ${themeName} build      # Compiler le thème`);
log(`   pnpm --filter ${themeName} prettier   # Vérifier le formatage`);
log(`   pnpm --filter ${themeName} stylelint  # Vérifier les styles\n`);
